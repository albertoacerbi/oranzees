...
```{r eval=FALSE}
tic()
test_05_0 <- run_oranzees(t_max = 12000, opt = 0, alpha_g = 0.5, init_world = 1, n_run = 20)
toc()
write_csv(test_05_0, "output_test/test_0.5_0.csv")
```
...
The two conditions that produce a better match for the pattern "D", also produce a good match for pattern "C" (absence explained by ecological factors), but less for pattern "A" (patterns absent at no site) and "B" (patterns not achieving habitual frequencies at any site).

***
### Code modification 1:

During the tests a bug was discovered and the code in [main.R](main.R) changed accordingly:
