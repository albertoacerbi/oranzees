...
The winning behaviours are generally similar in different runs with the same set-up of `oranzees_environment`, suggesting a relatively strong effect of the initalisation of $p_g$. We can check it directly, and see whether the winning behaviours are indeed the ones with higher $p_g$:
...
tibble(p_g = genes$V1, total = colSums(results), category = as_factor(c(rep("play", 4), rep("display", 4), rep("groom", 4), rep("courthsip", 4)))) %>%
  ggplot() +
  geom_point(aes(y = total, x = p_g)) +
  facet_wrap(~category) +
  theme_bw()
```

This is not a surprising result, as genetic propensity is the only directional force in the model, but why we may think about possible options to have genetic influence with a minor role.
