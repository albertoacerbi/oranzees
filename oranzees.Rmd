...
A second check exploits the fact that both $p_e$ and $p_g$ are assignated to the six populations following a geographical gradient. We would expect, thus, that populations that are geographically close to each other are also more similar genetically and they live in a similar environment. In other words, we expect for example populations 1 and 2 to be more similar one to each other than to populations 3, (or 4, 5, and 6).

We can check this with some quick scatterplots. Here the correlation between the genetic propensities of popualtions 1 and 2 (black dots) and 1 and 3 (red dots) for all 38 behaviours:
...
plot(oranzees_environment[oranzees_environment$population == 1, ]$p_g, oranzees_environment[oranzees_environment$population == 2, ]$p_g, pch = 19, 
      xlab = "Population 1", ylab = "Population 2", xlim = c(0, 1), ylim = c(0, 1))
