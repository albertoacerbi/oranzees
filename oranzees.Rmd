...
Following again Whiten et al., 1999, we consider 38 possible behaviours. The exact characteristics of these behaviours will be examined later. For now, we implement the idea that the probability to acquire any of these behaviours depends (among other things) by two factors:
...
These probabilities are assigned randomly and independently to each behaviours. To assign them to the different populations we use a geographical gradient. For each behaviour, a random point is chosen in the Cartesian space and its distance to each population is calculated. This distance is then normalised between 0 and 1000 for the genetic propensity and between 0 and 500 for the ecological availability. The actual probabilities are then calculated as $1-d_{norm}$.
...
Since we are working with the genetic propensity, we normalized the distance by 1000, and we then find the final probability subtracting from 1 (values that are below 0, i.e. population that were at more than 1000 units distance, are considered 0):
...
Each line of the tibble records the genetic propensity and the ecological availability for each behaviour, for each population. Since there are 38 behaviours and 6 populations, the tibble has 228 lines. 
...
As we wanted, values of $p_e$ are lower, and a few of them are zeros. 
...
plot(oranzees_environment[oranzees_environment$population == 1, ]$p_g, oranzees_environment[oranzees_environment$population == 2, ]$p_g,
  pch = 19,
  xlab = "Population 1", ylab = "Population check", xlim = c(0, 1), ylim = c(0, 1), col = alpha("black", 0.8)
)
points(oranzees_environment[oranzees_environment$population == 1, ]$p_g, oranzees_environment[oranzees_environment$population == 3, ]$p_g, pch = 19, col = alpha("red", 0.8))
grid()
```

And the same for the ecological availabilities (*explain in case the black ones are at zero*):
...
points(oranzees_environment[oranzees_environment$population == 1, ]$p_e, oranzees_environment[oranzees_environment$population == 3, ]$p_e, pch = 19, col = alpha("red", 0.8))
grid()
```
