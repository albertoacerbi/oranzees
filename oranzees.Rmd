...
Let's assume a population of $N=100$ individuals. This is a mock-up population that we will later use to model the real oranzees populations.
...
The probability of innovation, at each time step $t$, depends on $p_{state}$ and on the age of the individual. We simply assume that adult individuals are more conservative, and the probability calculated by $p_{state}$ is divided by two for oranzees that are older than 180 months, as described above. The probability, for the individual $i$ to innovate can be calculated as:
...
At this point, we can wrap all together in a function, and see what happens to our mock-up population. Let's declare again the variables we are interested in, and re-intialise all the data structures, to be sure in case we changed something before:
