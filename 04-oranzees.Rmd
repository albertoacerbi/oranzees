...
We should first run the code for all the fucntion we wrote, without showing it in the html rendering (remember the option`echo=FALSE`).
...
We can now do some basic tests to see that everything works. What happens, for example, if we run again 10 runs, but this time with `init_world = TRUE`? We should expect much more diversity between runs:

```{r}
my_test <- mockup_oranzees(t_max = 12000, alpha_g = 0.7, alpha_e = 0.9, init_world = TRUE, n_run = 10)
plot_multiple_runs(my_test = my_test, n_run = 10)
```

Let's go back to the usual `init_world = FALSE`. If we set `alpha_g = 0.5` and `alpha_e = 0.5` there is not any effect of genetic predisposition and ecological availability. 

```{r}
my_test <- mockup_oranzees(t_max = 12000, alpha_g = 0.5, alpha_e = 0.5, init_world = FALSE, n_run = 10)
plot_multiple_runs(my_test = my_test, n_run = 10)
```

Finally, we can do the opposite, and set both to $1$, for maximum effect:

```{r}
my_test <- mockup_oranzees(t_max = 12000, alpha_g = 1, alpha_e = 1, init_world = FALSE, n_run = 10)
plot_multiple_runs(my_test = my_test, n_run = 10)
```
