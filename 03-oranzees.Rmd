...
Now that we had implemented all behaviours, we can put everything together, so to have a code to test extensivley what happens in our mock-up population, before writing the simulation for the 'real' oranzees.
...
On my laptop, the execution time is around 11 seconds. Of course, this can vary according to different hardware, what is important is not the absolute time, but the ratio between this time and the vectorised code. To vectorise the code, we need to replace most of what is iterated in the `for (i in 1:N)` cycle, applying it directly to the `pop` matrix. Let's initalise again a popualtion, outside of the pre-written function.
...
test_oranzees3_better <- function(t_max) {
  ...
  for (t in 1:t_max) {
    ...
    pop[old[dead], ] <- 0
    # innovation bit:
    nut_y <- rowSums(pop[, 17:20]) + rowSums(pop[, 25:27]) + rowSums(pop[, 31:32]) + pop[, 35] + pop[, 37]
    nut_z <- rowSums(pop[, 21:24]) + rowSums(pop[, 28:30]) + rowSums(pop[, 33:34]) + pop[, 36] + pop[, 38]
    state <- (sum(nut_y + nut_z) - abs(nut_y - nut_z)) / 10 
    p_state <- runif(N) < rnorm(N, mean = 1 - state, sd = 0.05)
    p_peering <- rnorm(22, mean = colSums(pop[, 17:38]), sd = 1)
    p_peering[p_peering < 0] <- 0
...
Now, this is around 15 or 20 times faster! 
