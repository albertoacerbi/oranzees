...
We assume that a time step $t$ of the simulation represents a month. Demography is very simple: from when they are 25 years old, there is a 1% probability an oranzee will die each month, or they die when they are 60 years old. The number of individuals in the population is fixed, so each time an oranzee dies is replaced by a newborn. At the beginning of the simulations, the populations are randomly initialised with individuals between 0 and 25 years (i.e. 300 months) old.
