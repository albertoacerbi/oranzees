...
Ecological availability has influence only on food-related behaviours. Ecological availability can be thought as a probability $p_e(0,1)$ that represents the likelihood of finding a resource, or its nutritional value, in each site. This probability is assigned randomly and independently to each behaviour. A parameter of the model, $\alpha_e$, determines the probability that the ecological availability of each behaviour is equal for all the six populations, or if it is different. 
...
In addition, each behaviour has an associated $p_g(0,1)$, that stands for the genetic propensity to express this behaviour. $p_g$ is assigned to *all* behaviours, both in the social and in the food-related category, and it is calcualted in the same way of $p_e$, using the parameter $\alpha_g$ to determine the probability of genetic propensity being different. Notice that $\alpha_g=0$ does not mean that there are not genetic influences on the behaviours, but that there are no *differences* between the populations with this respect.
