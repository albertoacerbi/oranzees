...
Everything is similar as before, but now the 'unbalance' value is not substracted from $1$, but from the total amount of foodstuff, and all divided by the maxmium possible amount, i.e. $10$.
...
Also, having categories of behaviours of different size (for social behaviours they are all categories of four behaviours) is likely to create possibly unwanted effects. In particular, as behaviours are discarded only when a new behaviour in the same category is innovated, behaviours that are the only one for their category, like the last four food-related behaviours, will be advantaged, as long as their $p_g * p_e > 0$
