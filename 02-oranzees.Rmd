...
for (i in 1:N) {
  ...
  if (length(food_behaviours) > 1) {
    ...
    nutrients <- test_environment$nutrient[food_behaviours]
    if (length(unique(nutrients)) > 1) {
      state[i] <- ((1 - abs(sum(amount[nutrients == "Y"]) - sum(amount[nutrients == "Z"])) / 5) + sum(amount / 5)) / 2
    } else {
      state[i] <- 0
    }
  } else {
...
However, since the calculation of the state for food-related behaviours is more complex, is worth to do some tests to check that everything works as we want. Let's buil a custom `test_environment$p_e` that we can use for the test:
...
As well as some custom individuals in the popualtion that cover some possible cases. We can build them looking at the table above. For example, we want an individual that has an optimal and balances diet:
